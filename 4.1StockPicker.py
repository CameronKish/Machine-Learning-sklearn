...
def Build_Data_Set():
    data_df = pd.read_csv("key_stats_acc_perf_WITH_NA.csv") #with or w/o get similar results
...
def Analysis():
    ...
    data_df = data_df.replace(np.nan,0).fillna(0)
    ...
    for i in range(len(X)): 
        p = clf.predict(X[[i]])[0] #prediction
        if p == 1: #if we predict it will outperform...
                print(Z[i])
